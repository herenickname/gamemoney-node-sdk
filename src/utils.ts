...
const publicCert = fs.readFileSync(new URL('certs/gm.crt', pathToFileURL(__filename))).toString()
...
export function generateRsaSignature(body: Json, key: RsaKey): string {
    return crypto.createSign('RSA-SHA256').update(parametersToString(body), 'utf8').sign(key, 'base64')
}

export function generateHmacSignature(body: Json, key: HmacKey): string {
    return crypto.createHmac('sha256', key).update(parametersToString(body), 'utf8').digest('hex')
}
